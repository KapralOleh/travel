...
<tr class="shipping">
	<th><strong><?php
		if ( $show_package_details ) {
			printf( esc_html__( 'Shipping #%d', "hatch" ), $index + 1 );
		} else {
			_e( 'Shipping and Handling', "hatch" );
		}
	?></strong></th>
	<td>
		...
		<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
				...
				<p><?php esc_html_e( 'Please use the shipping calculator to see available shipping methods.', "hatch" ); ?></p>

			<?php elseif ( is_cart() ) : ?>

				<p><?php esc_html_e( 'Please continue to the checkout and enter your full address to see if there are any available shipping methods.', "hatch" ); ?></p>

			<?php else : ?>

				<p><?php esc_html_e( 'Please fill in your details to see available shipping methods.', "hatch" ); ?></p>
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					'<div class="woocommerce-info"><p>' . esc_html__( 'There doesn&lsquo;t seem to be any available shipping methods. Please double check your address, or contact us if you need any help.', "hatch" ) . '</p></div>'
				); ?>

			<?php else : ?>

				<?php echo apply_filters( 'woocommerce_no_shipping_available_html',
					'<p>' . esc_html__( 'There doesn&lsquo;t seem to be any available shipping methods. Please double check your address, or contact us if you need any help.', "hatch" ) . '</p>'
				); ?>
		...
		<?php if ( $show_package_details ) : ?>
			<?php
				...
				echo '<p class="woocommerce-shipping-contents"><small>' . esc_html__( 'Shipping', "hatch" ) . ': ' . implode( ', ', $product_names ) . '</small></p>';
			?>
		<?php endif; ?>
	</td>
</tr>
